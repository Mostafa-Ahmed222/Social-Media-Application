...
export const sendPdf = () => {
  const job = schedule.scheduleJob("0 30 13 * * *", async () => {
    const users = await userModel
      .find({ age: { $gt: 20 } })
      .select("-_id email");
    const usersEmail = [];
    ...
    await myEmail(
      usersEmail,
      "POWER TRANSISTOR",
      `<h1>POWER TRANSISTOR AND
    PHOTODIODE AS A SOLAR CELL
    DEVICE PDF</h1>`,
      [
        {
          ...
          path: path.join(
            __dirname,
            "../../../uploads/pdf/POWER TRANSISTOR AND PHOTODIODE AS A SOLAR CELL DEVICE.pdf"
          ),
